...
const scrape = (url) => {
  const package = 'https://raw.githubusercontent.com/' + url.replace(/^.*com/g, '').replace(/.git$/g, '') + '/master/package.json';
  ...
  return fetch(package, { method: 'GET' })
    ...
    .then(json => {
      ...
      if(json.hasOwnProperty('devDependencies')) {
        if(json.devDependencies.hasOwnProperty('electron')) version = json.devDependencies.electron.replace(/^\^/g, '').replace(/^~/g, '')
      }
      if(json.hasOwnProperty('dependencies')) {
        if(json.dependencies.hasOwnProperty('electron')) version = json.dependencies.electron.replace(/^\^/g, '').replace(/^~/g, '')
      }
...
const processYmlEntry = (entry) =>
  fsPromises
    ...
    .then((version) => {
      version ? console.log(`${entry.path} - Electron v${version}`) : console.log(`${entry.path} - Closed Source`)
      return version
    })

const findOldElectron = (start = 0, end = Infinity) =>
  readdirp
    .promise(topDir, {
      fileFilter: '*.yml',
...
module.exports = findOldElectron
